...
public interface TemperatureRepository extends JpaRepository<Temperature, Long> {
    ...
    @Query("SELECT new ongi.temperature.dto.FamilyTemperatureDailyResponse$DailyTemperature(CAST(t.createdAt AS date), SUM(t.temperature)) " +
            "FROM Temperature t WHERE t.familyId = :familyId GROUP BY CAST(t.createdAt AS date) ORDER BY CAST(t.createdAt AS date) DESC")
    List<FamilyTemperatureDailyResponse.DailyTemperature> getFamilyTemperatureDaily(@Param("familyId") String familyId, int days);
    ...
    @Query("SELECT new ongi.temperature.dto.FamilyTemperatureContributionResponse$Contribution(CAST(t.createdAt AS date), t.userId, SUM(t.temperature)) " +
            "FROM Temperature t WHERE t.familyId = :familyId GROUP BY CAST(t.createdAt AS date), t.userId ORDER BY CAST(t.createdAt AS date) DESC")
    List<FamilyTemperatureContributionResponse.Contribution> getFamilyTemperatureContributions(@Param("familyId") String familyId, int days);
} 